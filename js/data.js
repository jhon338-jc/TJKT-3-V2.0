...
const defaultData = {
    siswa: [
        ...
        { id: 8, nama: "Farrsa Dafa", nisn: "0088459839", foto: "L1" },
...
// Inisialisasi data
function initData() {
    if (!localStorage.getItem('db_tjkt')) {
        localStorage.setItem('db_tjkt', JSON.stringify(defaultData));
    }
}
...
function getData() {
    initData();
    return JSON.parse(localStorage.getItem('db_tjkt'));
}
...
function addSiswa(siswa) {
    const data = getData();
    siswa.id = getNextId(data.siswa);
    data.siswa.push(siswa);
    saveData(data);
    return siswa;
}
function updateSiswa(id, newData) {
    const data = getData();
    const index = data.siswa.findIndex(s => s.id === id);
    if (index !== -1) {
        data.siswa[index] = { ...data.siswa[index], ...newData };
        saveData(data);
        return true;
    }
    return false;
...
function updateStruktur(id, newData) {
    const data = getData();
    const index = data.struktur.findIndex(s => s.id === id);
    if (index !== -1) {
        data.struktur[index] = { ...data.struktur[index], ...newData };
        saveData(data);
        return true;
    }
    return false;
...
function addGaleri(item) {
    const data = getData();
    item.id = getNextId(data.galeri);
...
function addMusik(item) {
    const data = getData();
    item.id = getNextId(data.musik);
    data.musik.push(item);
    saveData(data);
    return item;
}
